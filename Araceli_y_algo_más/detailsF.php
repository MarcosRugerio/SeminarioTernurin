...
if ($id == '' || $token == '') {
    ...
    if ($token == $token_tmp) {
        ...
        if ($sql->fetchColumn() > 0) {
            $sql = $con->prepare("SELECT nombre, ruta_img, precio1, precio2, precio3 FROM productos WHERE id=?  AND categoria_id=2 LIMIT 1");
            $sql->execute([$id]);
        ...
        <div class="container">
            ...
            <div class="card mb-3 text-center" style="border-color: white;">
                <div class="row g-0">
                    ...
                    <div class="col-md-5">
                        <img class="card-img" src="img/productos/BebidasF/<?php echo $ruta_img; ?>">
                    </div>
                </div>
            </div>
        </div>
